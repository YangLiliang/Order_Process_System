...
public:
	...
	// 根据新订单请求做出应答消息
	void processNewOrder(const NewOrderRequest&, ServerReaderWriter<ExecutionReport, NewOrderRequest>*);
	// 根据撤销订单请求做出应答消息
...
private:
	...
	// 卖订单
	void sellOrders(const uint64_t&, const std::string&);
	// 买订单
	void buyOrders(const uint64_t&, const std::string&);
	// 将订单加入至待售卖容器
