...
void TradingMarket::processCancelOrder(const CancelOrderRequest& request, ExecutionReport& report){
	...
	std::lock_guard<std::mutex> cm(cancel_mutex);
	// 判断orderID是否存在
...
// 根据查询订单请求做出应答消息
void TradingMarket::processQueryOrder(const QueryOrderRequest& request, std::vector<NewOrderRequest>& reports){
	// 对orders的拷贝操作加锁，作用域结束自动解锁
	std::lock_guard<std::mutex> cm(cancel_mutex);
	reports.reserve(orders.size());
	for(auto& order:orders){
		reports.push_back(order.second);
	}
}

// 卖订单操作
void TradingMarket::sellOrders(const uint64_t& orderID, const std::string& stockID, std::vector<std::pair<uint64_t, ExecutionReport> >& reports){
	// 获取卖订单
...
// 买订单操作
void TradingMarket::buyOrders(const uint64_t& orderID, const std::string& stockID, std::vector<std::pair<uint64_t, ExecutionReport> >& reports){
	// 获取买订单
